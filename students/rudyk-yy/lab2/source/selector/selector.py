...
class Selector:
    ...
    def select(self):
        ...
        omega = list(range(len(self.X)))  # индексы эталонов
        improved = True
        ...
        while improved and len(omega) > 1:
            ...
            for i in omega:
                ...
                ccv_new = C.CCV(np.array(omega_new),k=self.k)
    ...
    def select_add(self):
        ...
        # --- начальное множество Ω: по одному объекту каждого класса
        ...
        while improved:
            ...
            # --- ищем лучший объект для добавления
            for i in range(len(self.X)):
                ...
                print(f"\rПробуем добавить {i}: CCV={ccv_new:.6f}", end="")
        ...
        if self.verbose:
            print(f"\nГотово! Итоговое Ω содержит {len(Omega)} элементов, CCV={self.best_ccv:.6f}")
        return np.array(Omega)
    ...
    def plot_history(self):
        ...
        plt.ylabel("CCV(Ω)")
        plt.title("Изменение CCV при жадном удалении эталонов")
